...
## Deauthorization / data compliance (REQUIRED to publish)

A **published** app must expose a deauthorization endpoint — Marketplace review fails without
one (`BACKLOG.md` → OPS-3). The server implements it at
[`server/src/zoom/deauth.js`](src/zoom/deauth.js).
...
- **Secret Token** (Marketplace → your app → the Feature/Webhook section): copy it into the
  **`ZOOM_WEBHOOK_SECRET_TOKEN`** env var for the matching deployment. It is **not** the
  ...
  (`x-zm-signature` over `v0:<timestamp>:<raw body>`).
...
- On uninstall, Zoom POSTs `app_deauthorized`; the endpoint verifies the signature and POSTs the
  confirmation to `https://api.zoom.us/oauth/data/compliance` (Basic auth with the client
  id/secret, `compliance_completed: true`).
