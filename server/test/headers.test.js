...
test('CSP allows the app bundle and Zoom embedding, with connect-src pinned', async () => {
  ...
  try {
    ...
    // connect-src is pinned to self + Zoom hosts: NOT the dev-permissive wss:/any-https:.
    assert.ok(
      csp.includes("connect-src 'self' https://*.zoom.us https://*.zoom.com"),
      'connect-src pinned to self + Zoom hosts'
    );
